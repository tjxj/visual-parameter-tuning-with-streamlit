...
plt.text(0.5, 0.3, 'ROC curve (area=%0.2f)' % roc_auc_score(y_test, probs[:, -1]))
...
print(classification_report(y_test, np.where(probs[:, -1] > 0.5, 1, 0)))

print('-----------------混淆矩阵-----------------------')

print(confusion_matrix(y_test, np.where(probs[:, -1] > 0.5, 1, 0)))
